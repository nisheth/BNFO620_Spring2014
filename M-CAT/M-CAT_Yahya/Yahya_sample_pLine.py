...
def runBowtiePair(infile1,infile2,index,samfile):
    if not isFileThere(samfile) : 
        cmd = "/home/bnfo620/bin/bowtie2-2.0.6/bowtie2 --very-sensitive-local -p 2 -1 " + infile1 + " -2 " + infile2 + " -x " + index + " -S " + samfile
        runSystemCMD(cmd)
...
def sortBam(bamfile,sortedbam):
    if not isFileThere(sortedbam):
        cmd = "/home/bnfo620/bin/samtools sort -no " + bamfile + " abc " + "> " + sortedbam
        print cmd, "sort"
        runSystemCMD(cmd)
...
def main(): 
    ...
    sortedbam = outdir + "/" + projectname + "_bowtie_sorted.bam"
