...
my ($max_score, $OTU_word_abundance, $word_score);
my $num_of_reads = 1;
my $Total_OTUs = 1;
my $uniq = 0;
my $end = 0;
...
if ($max_score >= $thresh) {
Modify_OTU($OTU, $seq, $abundance, $particular_seq_word_list);
}
else{
Create_OTU($seq, $abundance, $particular_seq_word_list);
}
...
my $total_words_in_OTU = $OTU_Hash{$OTU}{OTU_total_word_count};
$OTU_sum = 0;

if(!defined($total_words_in_OTU)) { 
	$total_words_in_OTU = length($seq) - $arb_seven;
}
print SCORE ("OTU:", $OTU, "\t", "Words_in_OTU: ", $total_words_in_OTU, "\n");
	...
	if (exists $OTU_Hash{$OTU}{Seed}) {
		$seed_length = length($OTU_Hash{$OTU}{Seed}) ;
	   # print "$OTU\t$read_length\t$seed_length\n"; 
...
sub Modify_OTU {
my ($OTU, $seq, $abundance, $particular_seq_word_list) = @_ ;
#print "Modifying OTU ... nihar\n";
$OTU_Hash{$OTU}{seq_count} += 1;
	...
	foreach my $word (@$particular_seq_word_list) {   ####### ?!@#?!@$!@$?!@$?!@$?!$@
	 if ($word ne "") {
		...
		$OTU_Hash{$OTU}{OTU_total_word_count} +=$abundance;
			if (exists $OTU_Hash{$OTU}{$word} ) {
			$OTU_Hash{$OTU}{$word}{word_abundance} += $abundance;
			}
			else {
			$OTU_Hash{$OTU}{$word}{word_abundance} = $abundance;
			}
	...
	#foreach my $word (@$particular_seq_word_list) {   ####### ?!@#?!@$!@$?!@$?!@$?!$@
		...
		#$OTU_Hash{$OTU}{OTU_total_word_count} +=$abundance;
		#$OTU_Hash{$OTU}{$word}{word_abundance} += $abundance;
		print WORD ($OTU, "\t", $word, "\t", $OTU_Hash{$OTU}{$word}{word_abundance}, "\n");
		}
	 }
	
	print WORD "_____________________________________________________________","\n";
	...
	foreach my $OTU (%OTU_Hash) {
		if ($SeqName ne "") {
	#	print OTU ($OTU, "\n", @{$OTU_Hash{$OTU}{name}}, "\n");
		#print FREQ ($OTU, "\t", $OTU_Hash{$OTU}{seq_count}, "\t", $OTU_Hash{$OTU}{best}, "\t", $OTU_Hash{$OTU}{max}, "\n");
