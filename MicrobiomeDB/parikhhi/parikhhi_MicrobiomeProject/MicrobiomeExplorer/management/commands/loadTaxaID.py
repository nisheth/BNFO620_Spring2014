...
from MicrobiomeExplorer.models import Project, Sample, SampleVariable, Read, ClassificationMethod, TaxaID
import sys

class Command(BaseCommand):
	args = '<adminFile>'              #arguments from command-line
	help = 'Loads flat file of TaxaIDs into the database, to run use the command: python manage.py loadTaxaID <filename>'       #message displayed upon help command

	def handle(self, *args, **options):
		for filename in args:       #for each of the files
			try:                    #try to open the file
				TaxaIDList = csv.DictReader(open(filename,"rb"),delimiter='\t')      #open the file
			except csv.Error, e:    #if it fails throw an error
				print e

		self.stdout.write("Loading TaxaIDs ...")
