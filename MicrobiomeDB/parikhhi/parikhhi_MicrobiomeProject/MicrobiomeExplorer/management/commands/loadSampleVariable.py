...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		for eachsamplevar in samplevarList:     #for each row in the file (row is a dict using the header row as the keys and the values in the following rows as the values)
			...
			sample = Sample.objects.get(name=eachsamplevar['SampleName'])
			print sample
			samplevariable = SampleVariable.createSampleVariable(sample, eachsamplevar['SampleVariable'], eachsamplevar['VariableValue'])
